...
export default function Home() {
  ...
  const loadScenes = async () => {
    try {
      ...
      const response = await fetch(`http://0.0.0.0:8000/api/scenes?${params}`)
      if (response.ok) {
  ...
  const loadModelPreferences = async () => {
    try {
      const response = await fetch('http://0.0.0.0:8000/api/models/preferences')
      if (response.ok) {
        const data = await response.json()
  ...
  const handleIndexFiles = async () => {
    try {
      const response = await fetch('http://0.0.0.0:8000/api/ingest/index', {
        method: 'POST',
  ...
  const handleModelChange = async (agentName: string, modelId: string) => {
    try {
      const response = await fetch('http://0.0.0.0:8000/api/models/agent-config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const processScene = async (sceneId: string) => {
    ...
    try {
      const response = await fetch('http://0.0.0.0:8000/api/patches/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          scene_id: sceneId,
